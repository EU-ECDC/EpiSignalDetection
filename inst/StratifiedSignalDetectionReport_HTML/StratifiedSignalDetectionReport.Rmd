---
title: "Stratified report of alarms "
subtitle: "`r params$disease` cases in `r params$country` (`r params$indicator`) "
...
params:
  ...
  unit: "Month"
  daterange: !r c("2010-01-01", "2016-12-31")
...
* There are `r sum(datasetTP$NumValue, na.rm=TRUE)` cases reported in the alarm detection period, all strata added up.
...
This report is stratified according to the stratification variable 'Population'.
...
* The selected strata are those with at least one reported case during the alarm detection period.
* There are `r length(strata)` strata with at least one case in the alarm detection period: `r paste(strata, collapse = ", ")`.

In decreasing order of number of cases, the most frequent serotypes on the alarm detecion period are:
...
DT::datatable(tab, 
              options = list(
                ...
                columnDefs = list(list(className = 'dt-left', orderable = TRUE, targets = 1))),
...
for(i in 1:length(countries)){
  ...
  for(j in 1:length(strata)){
    ...
    dataset_1 = dplyr::filter(dataset, RegionName == country & Population == stratum)
...
```{r AlarmTableByGeo, eval = TRUE, results = 'hide'}
...
```{r AlarmTableByPop, eval = TRUE, results = 'hide'}

out=NULL

for(i in strata){  
  Result_c <- dplyr::filter(Result, Stratum == i & grepl("Alarm", ALGO))
  row.names(Result_c) <- NULL
  names(Result_c) <- c(paste("Time (agg. by ", tolower(input$unit), ")", sep = ""),
                       "Place",
                       "Stratum",
